...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (const file of buildResponse.files) {
      ...
      const blocked = [".env", ".env.local", "package.json", "next.config.ts", ".git"];
      if (blocked.some((b) => file.path === b || file.path.startsWith(".git/"))) {
    ...
    const commitSummary = prompt.length > 60 ? prompt.slice(0, 57) + "..." : prompt;
