...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await (client.messages.create as any)({
      ...
      system: `You are Jarvis, Dylan Murdock's AI chief of staff at dylan@narwhalhomes.com. Generate morning briefing as JSON: {"date":"${dateStr}","greeting":"","email_summary":"","urgent_emails":[{"from":"","subject":"","action":""}],"top_priorities":["","",""],"business_update":"","pending_approvals":${approvals?.length||0},"active_projects":${projects?.length||0}}`,
