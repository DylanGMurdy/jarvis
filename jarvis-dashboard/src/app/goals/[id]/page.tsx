...
export default function GoalDetailPage({
  ...
  useEffect(() => {
    ...
    setGoal(g);
    if (g) {
      setJournal(db.goalJournal.list(id));
    }
    setLoaded(true);
  }, [id]);

  function reload() {
    setGoal(db.goals.get(id));
    setJournal(db.goalJournal.list(id));
  }
  ...
  function toggleMilestone(milestoneId: string) {
    ...
    db.goals.update(id, { milestones: updated, progress: newProgress });
    reload();
  }
  ...
  function saveJournalEntry() {
    ...
    db.goalJournal.create({ goal_id: id, entry: journalInput.trim() });
    setJournalInput("");
    reload();
  }
  ...
  async function sendChat(content?: string) {
    ...
    try {
      ...
      setChatMessages([
        ...next,
        { role: "assistant", content: "Failed to reach JARVIS. Try again." },
      ]);
    } finally {
  ...
  const suggestedPrompts = [
    ...
    "Help me break down the next milestone",
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-[#e2e8f0]">
      <div className="max-w-[1200px] mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          <Link
            href="/"
            className="inline-flex items-center gap-2 text-[#64748b] hover:text-[#e2e8f0] transition-colors mb-6"
          >
            <svg
              className="w-4 h-4"
              fill="none"
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#6366f1]/20 text-[#818cf8] border border-[#6366f1]/30">
                  {goal.category}
                </span>
              </div>
            </div>
          ...
          <div className="space-y-2">
            {goal.milestones.map((m) => (
              <button
                ...
                className="w-full flex items-center gap-3 p-4 bg-[#12121a] border border-[#1e1e2e] rounded-xl hover:border-[#6366f1]/40 transition-all text-left cursor-pointer"
              >
                <div
            ...
            {goal.weekly_breakdown.map((week, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={
                      isCurrent ? "text-[#e2e8f0] font-medium" : "text-[#94a3b8]"
                    }
                  >
                    {week}
                  </span>
                  {isCurrent && (
                    <span className="ml-auto flex-shrink-0 px-2 py-0.5 text-xs font-medium rounded-full bg-[#6366f1]/20 text-[#818cf8]">
                      Current
                    </span>
                  )}
                </div>
        ...
        <section className="mb-10">
          ...
          <div className="bg-[#12121a] border border-[#1e1e2e] rounded-xl p-4 mb-4">
            <textarea
              ...
              rows={3}
              className="w-full bg-transparent text-[#e2e8f0] placeholder-[#64748b] resize-none outline-none"
            />
            <div className="flex justify-end mt-2">
              <button
                onClick={saveJournalEntry}
                disabled={!journalInput.trim()}
                className="px-4 py-2 bg-[#6366f1] text-white text-sm font-medium rounded-lg hover:bg-[#5558e6] disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer"
              >
                Save Entry
              </button>
            </div>
          ...
          <div className="space-y-3">
            {[...journal].reverse().map((entry) => (
              ...
              >
                <p className="text-[#e2e8f0] whitespace-pre-wrap mb-2">
                  {entry.entry}
                </p>
          ...
          <div className="bg-[#12121a] border border-[#1e1e2e] rounded-xl overflow-hidden">
            ...
            <div className="p-4 space-y-3 max-h-[400px] overflow-y-auto">
              {chatMessages.length === 0 && (
                <p className="text-[#64748b] text-sm text-center py-8">
                  Ask JARVIS anything about your goal progress, strategy, or next
                  steps.
                </p>
              )}
              ...
              {chatLoading && (
                <div className="flex justify-start">
                  <div className="bg-[#1e1e2e] text-[#64748b] px-4 py-2.5 rounded-xl text-sm">
                    Thinking...
                  </div>
                </div>
              )}
              ...
              <div className="px-4 pb-3 flex flex-wrap gap-2">
                {suggestedPrompts.map((prompt) => (
                  <button
                    ...
                    className="px-3 py-1.5 text-xs bg-[#1e1e2e] text-[#94a3b8] rounded-lg hover:bg-[#6366f1]/20 hover:text-[#818cf8] transition-colors cursor-pointer"
                  >
                    {prompt}
                  </button>
                ))}
            ...
            <div className="border-t border-[#1e1e2e] p-3 flex gap-2">
              <input
                ...
                className="flex-1 bg-[#0a0a0f] border border-[#1e1e2e] text-[#e2e8f0] placeholder-[#64748b] px-4 py-2.5 rounded-lg outline-none focus:border-[#6366f1]/50 transition-colors"
              />
              <button
                onClick={() => sendChat()}
                disabled={!chatInput.trim() || chatLoading}
                className="px-4 py-2.5 bg-[#6366f1] text-white rounded-lg hover:bg-[#5558e6] disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
